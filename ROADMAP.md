...
2. **The population ledger** (Culture). A drill-down from the world to regions to countries, each level shown as a ledger for a chosen year: last year's population, plus births, minus deaths, plus net migration, equals this year's, with a year slider from 1950 to the UN's 2100 projection and each row clickable to go a level down. Source: UN World Population Prospects (population, births, deaths, net migration per country per year; refreshes every two years with annual estimates). Aha: the same ledger carries opposite signs in different places, so the "overpopulation" and "depopulation" arguments are both looking at true rows: Japan, Korea and Italy in decline with migration not covering the gap, Nigeria and Pakistan doubling, the US growing only on the migration line, world fertility heading below replacement. Version 1 is WPP only. Version 2 opens deaths into causes (WHO Global Health Estimates, open; not IHME GBD, whose terms don't suit a public site) and splits migration into gross in and out where the OECD migration database has it (OECD countries only; WPP gives net migration alone). *(design 2026-09-21)*
